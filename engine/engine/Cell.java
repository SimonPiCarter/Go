...
public class Cell {
	...
	public void setCellColor(Colors color) {
	...
	public String toString() {
		return "Cell[coordinate=(" + this.coordX + ";" + this.coordY + "), " + "color=" + cellColor + "]\n";
	}
	...
	public Cell cellNearby(Board board, Directions dir) {
		...
		case NORTH:
			if (this.coordX - 1 >= 0) {
				...
				return new Cell(Colors.BORDER);// gestion des bords
		case SOUTH:
	...
	private boolean group(Board board, Colors color) {
		...
		groupList.add(this);
		int liberties = computeNumberOfLiberties(board, groupList);

		if (liberties == 0) {
			for (int t = 0; t < groupList.size(); t++) {
				groupList.get(t).setDead(true);// on met les pierres qui sont
												// morte en état "mort"
			}
			return false;
		} else {
	...
	public int computeNumberOfLiberties(Board board, ArrayList<Cell> groupList) {
		int liberties = 0;
		for (int t = 0; t < groupList.size(); t++) {
		...
		for (int t = 0; liberties == 0 && t < groupList.size(); t++) {
			if (groupList.get(t).cellNearby(board, Directions.WEST).getCellColor() == Colors.EMPTY) {
				liberties++;
			}
			if (groupList.get(t).cellNearby(board, Directions.EAST).getCellColor() == Colors.EMPTY) {
				liberties++;
			}
			if (groupList.get(t).cellNearby(board, Directions.NORTH).getCellColor() == Colors.EMPTY) {
				liberties++;
			}
			if (groupList.get(t).cellNearby(board, Directions.SOUTH).getCellColor() == Colors.EMPTY) {
				liberties++;
			}
		}
		return liberties;
